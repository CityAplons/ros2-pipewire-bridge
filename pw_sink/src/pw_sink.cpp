...
{
  struct roundtrip_data * d = static_cast<struct roundtrip_data *>(data);
...
int main(int argc, char * argv[])
{
  ...
  printf("hello world pw_sink package\n");

  pw_init(&argc, &argv);

  loop = pw_main_loop_new(NULL /* properties */);
  context =
    pw_context_new(pw_main_loop_get_loop(loop), NULL /* properties */, 0 /* user_data size */);
